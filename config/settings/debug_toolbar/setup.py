...
def show_toolbar(request=None, *args, **kwargs) -> bool:
    ...
    if request is not None:
        ...
        # Prevent DJDT from breaking Swagger UI or Schema generation
        if path.startswith("/api/swagger-ui/") or path.startswith("/api/schema/") or path.startswith("/api/redoc/"):
            return False
